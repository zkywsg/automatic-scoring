...
def domain_specific_rescale(y_true, y_pred, set_ids):
    ...
    for k, i in enumerate(set_ids):
        ...
        if i == 1:
            ...
            minscore = 0
            maxscore = 25
            y9_true.append(y_true[k]*(maxscore-minscore) + minscore)
            y9_pred.append(y_pred[k]*(maxscore-minscore) + minscore)
        else:
            print "Set ID error"
    prompts_truescores = [np.around(y1_true), np.around(y2_true), np.around(y3_true), np.around(y4_true), \
                            np.around(y5_true), np.around(y6_true), np.around(y7_true), np.around(y8_true), np.around(y9_true)]
    prompts_predscores = [np.around(y1_pred), np.around(y2_pred), np.around(y3_pred), np.around(y4_pred), \
                            np.around(y5_pred), np.around(y6_pred), np.around(y7_pred), np.around(y8_pred), np.around(y9_pred)]
